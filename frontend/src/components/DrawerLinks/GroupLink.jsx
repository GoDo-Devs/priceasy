...
function GroupLink({ Icon, path, title, children }) {
  ...
  const isActive =
    children.filter((route) => route.path === router.pathname)[0] ||
  ...
  return (
    <Box key={path}>
      <Stack direction="row" onClick={() => setOpen(!open)}>
        <ListItem className="my-1" key={path} disablePadding>
          <ListItemButton component={NavLink} to={path} disableGutters>
      ...
      <Collapse in={open} timeout="auto" unmountOnExit>
        <Box ml={2} borderLeft={1} borderColor="secondary.main">
          {children
            ...
            .map(({ path: subPath, icon: Icon, label: subLabel }) => (
              <ListLink
                key={subPath}
                path={subPath}
                Icon={Icon}
